...
def generate_chart(csv_filename):
    ...
    # ==========================================
    # GRÁFICO 1: Rede vs Bitrate Selecionado
    # ==========================================
    ...
    # Adiciona os nomes das qualidades flutuando (ex: 720p, 480p)
    ...
    ax1.set_title("Política 2 (Buffer-Based): Análise Isolada", fontweight='bold')
    ax1.grid(True, linestyle=":", alpha=0.7)
    ax1.legend(loc="upper left")

    # ==========================================
    # GRÁFICO 2: Comportamento do Buffer
    # ==========================================
    ax2.plot(segments, buffer_s, label="Nível do Buffer", color="teal", linestyle="-", marker="s")
    
    # As linhas tracejadas que mostram onde o seu abr_v2.py toma as decisões!
    ax2.axhline(y=4.0, color='orange', linestyle='--', alpha=0.5, label="Limite Superior (Sobe Qualidade se tiver banda)")
    ax2.axhline(y=2.5, color='red', linestyle='--', alpha=0.5, label="Limite Inferior (Desce Qualidade)")
    ...
    plt.tight_layout()
    plt.savefig("chart_policy2.png", dpi=300)
    print("Gráfico 'chart_policy2.png' salvo com sucesso!")
    plt.show()
